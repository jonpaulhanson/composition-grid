...
export async function renderExportBlob(opts: ExportOptions): Promise<Blob> {
  ...
  if (mode === 'composite') {
    ...
    // The same filter string the preview uses, derived from the output width so the notan's
    // blur stays the same proportion of the picture as it is on screen.
